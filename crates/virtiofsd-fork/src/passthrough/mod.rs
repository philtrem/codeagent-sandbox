...
use crate::compat::fd_ops::{O_DIRECT, O_PATH_OR_RDONLY};
use crate::compat::types::{fallocate64, off64_t};
...
impl PassthroughFs {
    ...
    fn after_invalidating_path_refresh(&self, _inode_data: &InodeData) -> io::Result<()> {
        // proc_paths-based migration info refresh is not available on this platform
        Ok(())
    }
...
impl FileSystem for PassthroughFs {
    ...
    fn setattr(
        ...
        if valid.intersects(SetattrValid::UID | SetattrValid::GID) {
            ...
            #[cfg(target_os = "linux")]
            let res = unsafe {
                let empty = CStr::from_bytes_with_nul_unchecked(EMPTY_CSTR);
                libc::fchownat(
    ...
    fn rename(
        ...
        // Use compat layer for cross-platform renameat2 support
        let res = unsafe {
            crate::compat::rename_ops::safe_renameat2(
                old_file.as_raw_fd(),
                oldname.as_ptr(),
                new_file.as_raw_fd(),
                newname.as_ptr(),
    ...
    fn lseek(
        ...
        // Safe because this doesn't modify any memory and we check the return value.
        let res = unsafe { libc::lseek(fd, offset as off64_t, whence as libc::c_int) };
        if res < 0 {
            Err(io::Error::last_os_error())
        } else {
            Ok(res as u64)
