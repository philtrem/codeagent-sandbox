...
pub fn validate_path(path: &str, root: &Path) -> Result<PathBuf, StdioError> {
    ...
    let resolved = resolve_components(&full_path).ok_or_else(|| StdioError::PathOutsideRoot {
        path: path.to_string(),
    })?;

    let canonical_root =
        resolve_components(root).ok_or_else(|| StdioError::PathOutsideRoot {
            path: path.to_string(),
        })?;

    if !resolved.starts_with(&canonical_root) {
        return Err(StdioError::PathOutsideRoot {
            path: path.to_string(),
        });
    }
...
/// (e.g., `..` at the root of a relative path, or past a drive letter on Windows).
