...
//!
//! SCM_RIGHTS is a BSD-originated API that works on both Linux and macOS.
...
// MSG_NOSIGNAL is not available on macOS. Use 0 instead and rely on
// SO_NOSIGPIPE or process-level SIGPIPE handling.
...
// CMSG_LEN: on glibc Linux and macOS, cmsg_len is usize. On musl, it's u32.
...
// On glibc Linux, msg_iovlen is usize and msg_controllen is usize.
// On musl, msg_iovlen is i32 and msg_controllen is u32.
// On macOS, msg_iovlen is i32 and msg_controllen is u32 (socklen_t).
#[cfg(not(any(target_env = "musl", target_os = "macos")))]
...
impl CmsgBuffer {
    fn with_capacity(capacity: usize) -> CmsgBuffer {
        ...
        if capacity <= CMSG_BUFFER_INLINE_CAPACITY {
            ...
            CmsgBuffer::Heap(
                vec![
                    // SAFETY: cmsghdr is a plain C struct that can be safely zeroed.
                    unsafe { std::mem::zeroed::<cmsghdr>() };
...
fn raw_sendmsg<D: IntoIovec>(fd: RawFd, out_data: &[D], out_fds: &[RawFd]) -> Result<usize> {
    ...
    if !out_fds.is_empty() {
        // SAFETY: cmsghdr is a plain C struct; zeroed is safe.
        let mut cmsg: cmsghdr = unsafe { std::mem::zeroed() };
...
unsafe fn raw_recvmsg(
    ...
    while !cmsg_ptr.is_null() {
        ...
        if cmsg.cmsg_level == SOL_SOCKET && cmsg.cmsg_type == SCM_RIGHTS {
            let fds_count: usize = ((cmsg.cmsg_len as usize - CMSG_LEN!(0) as usize)) / size_of::<RawFd>();
